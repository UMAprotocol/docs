...
|STABLESPREAD| STABLESPREAD is defined as: min(max(A - B + 1, 0), 2), where A refers to an equally weighted basket of {UST, CUSD, BUSD}. B is MUSD. UST is TerraUSD, the interchain stablecoin connected by the Cosmos IBC. CUSD is Celo Dollar, the stablecoin on the Celo network. BUSD is Binance USD, a stablecoin issued by Paxos in partnership with Binance. MUSD is an autonomous and non-custodial stablecoin on mStable, representing a basket of stablecoins on Ethereum, namely USDT, USDC, TUSD, and DAI.| [UMIP-28](https://github.com/UMAprotocol/UMIPs/blob/master/UMIPs/umip-28.md)
